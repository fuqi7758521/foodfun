...
var oTable;
function chargesInit(){
	
	oTable = $('#chargesTable').dataTable({
		...
		"bProcessing": true,
		"bServerSide": true,
		"sAjaxSource": "<%=request.getContextPath() %>/admin/charge/chargeTable.html",
		"sServerMethod": "POST",
		"aoColumns": [
			{ "mData": "userRealname" },
			{ "mData": "money" },
			{ "mData": "chargedDateStr" },
			{ "mData": "confirmedDateStr" },
			{ "mDataProp": function(obj) {
				if(obj['status'] == 0){
					return '<i class="fa fa-exclamation-triangle" style="color:#FFB848"></i> <a chargeId="'+obj['id']+'" href="javascript:;" onclick="chargePass(this)">通过</a><a chargeId="'+obj['id']+'"href="javascript:;" onclick="chargeNotPass(this)"> 取消</a>';
...
function chargeNotPass(obj){
	var $this = $(obj);
	var chargeId = $this.attr('chargeId');
	var url = "<%=request.getContextPath() %>/admin/charge/update.html";
	$.post(url, {chargeLogId:chargeId, status:1}, function(result){
		if(result == 200){
			toastr.options = {"timeOut": 2000, "closeButton": true};
			toastr["success"]("操作成功");
			//oTable.fnPageChange('first', true);
