...
<div class="page-container">
				...
				<div class="col-md-12">
					...
					<div class="portlet box yellow">
						<div class="portlet-title">
							...
							<div class="actions">
								<a id="add_btn" data-toggle="modal" class="btn green" role="button" href="#addUser"><i class="fa fa-plus"></i> 添加</a>
								<a id="edit_btn" data-toggle="modal" class="btn red" role="button" href="javascript:;"><i class="fa fa-pencil"></i> 编辑</a>
							</div>
						</div>
	...
	<!-- BEGIN ADD USER-->
	<div id="addUser" class="modal fade" role="basic" tabindex="-1">
					...
					<div class="modal-body">
							...
							<div class="form-group">
								<div class="input-icon ">
									...
									<input type="text" placeholder="实名" class="form-control" name="realname">
								</div>
							</div>
							<div class="form-group">
								<div class="input-icon ">
							...
							<div class="form-group">
								<div class="input-icon">
									<i class="fa fa-key"></i>
									<input type="password"  value="123456" placeholder="密码" class="form-control" name="password">
								</div>
							</div>
					</div>
	...
	<!-- BEGIN EDIT USER-->
	<div id="editUser" class="modal fade" role="basic" tabindex="-1">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
				...
				<form id="editForm" class="form-body" action="#" role="from">
					<div class="modal-body">
							...
							<div class="form-group">
								<div class="input-icon">
									<i class="fa fa-user"></i>
									<input id="username" type="text" readonly placeholder="用户名" class="form-control" name="username">
								</div>
							</div>
							<div class="form-group">
								<div class="input-icon ">
									<i class="fa fa-male"></i>
									<input id="realname" type="text" placeholder="实名" class="form-control" name="realname">
								</div>
							</div>
							<div class="form-group">
								<div class="input-icon ">
							...
							<div class="form-group">
								<div class="input-icon">
									<i class="fa fa-cny"></i>
									<input id="balance" type="text" placeholder="余额" class="form-control" name="balance">
								</div>
							</div>
							<div class="form-group">
...
function usersInit(){
	...
	oTable = $('#usersTable').dataTable({
		...
		"bProcessing": true,
		"bServerSide": true,
		"sAjaxSource": "<%=request.getContextPath() %>/admin/user/listTable.html",
		"sServerMethod": "POST",
		"aoColumns": [
			{ "mData": "realname" },
			{ "mData": "username" },
			{ "mData": "balance" },
			{ "mData": "registeredDateStr"},
	...
	$("#addForm").validate({
		...
		submitHandler: function (form) {
			$.ajax({
				url: '<%=request.getContextPath() %>/admin/user/addOrUpdate.html',
				type: 'POST',
	...
	$("#edit_btn").click(function () {
        ...
        if (trData!=null) {
            $("#editUser").modal('show');
            $("#userId").val(trData["id"]);
            $("#username").val(trData["username"]);
            $("#realname").val(trData["realname"]);
            $("#email").val(trData["email"]);
            $("#balance").val(trData["balance"]);
            $("#status").val(trData["status"]);
            $("#isAdmin").val(trData["isAdmin"]);
        }
	});
}
</script>
